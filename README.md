...
```bash
aws-sam-kts$ sam build
aws-sam-kts$ sam deploy --guided
```
...
```bash
aws-sam-kts$ sam local invoke HelloWorldFunction --event HelloWorldFunction/src/test/resources/events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-sam-kts$ sam local start-api
aws-sam-kts$ curl http://localhost:3000/
```
...
```bash
aws-sam-kts$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
aws-sam-kts$ cd HelloWorldFunction
